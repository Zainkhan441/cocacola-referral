...
type UseAdminUserProfileResult = {
...
// Live view of an arbitrary user's profile (not necessarily the caller's
// own) — only reachable by an admin per firestore.rules.
export function useAdminUserProfile(uid: string): UseAdminUserProfileResult {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      () => {
        setError("We couldn’t load this user’s profile. Please try again.");
        setLoading(false);
      },
    );
